...
const analyzeCVAgainstJDFlow = ai.defineFlow(
  ...
  async input => {
    ...
    const calculateScores = (reqs: Requirement[] | undefined, categoryName: keyof typeof PRIORITY_WEIGHTS) => {
        if (!reqs) return;
        reqs.forEach(req => {
            const weights = PRIORITY_WEIGHTS[categoryName];
            const basePoints = weights[req.priority];
            maxScore += basePoints;
            
            const alignmentDetail = output.alignmentDetails.find(
                detail => detail.requirement === req.description && detail.category === categoryName
            );
    ...
    const normalizeCategory = (category: string): keyof typeof PRIORITY_WEIGHTS => {
        if (category.toLowerCase().includes('education')) return 'Education';
        if (category.toLowerCase().includes('experience')) return 'Experience';
        if (category.toLowerCase().includes('certifica')) return 'Certification';
        if (category.toLowerCase().includes('technical')) return 'Technical Skill';
        if (category.toLowerCase().includes('soft')) return 'Soft Skill';
        if (category.toLowerCase().includes('responsibilit')) return 'Responsibility';
